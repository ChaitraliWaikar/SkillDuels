...
const BattleResults = () => {
  ...
  // Determine winner
  const isUserWinner = isSolo ? true : (userScore > opponentScore);
  const isDraw = isSolo ? false : (userScore === opponentScore);
  const winner = isSolo ? currentUser : (isDraw ? null : (isUserWinner ? currentUser : opponent));

  // Calculate XP and badge
  const calculatedXP = userScore * 10; // 10 XP per correct answer
  const finalXP = xpEarned || calculatedXP;
  ...
  const badge = badgeUnlocked || calculateBadge(userScore, totalQuestions);
  ...
  useEffect(() => {
    ...
    // Backend API call to save results
    // fetch('/api/battle/save-results', {
    //   method: 'POST',
  ...
  return (
    <div className="app-container">
      ...
      <div className="content-wrapper">
        ...
        <main className="results-main">
          {/* Celebration Animation - Simple and Elegant */}
          {showCelebration && (
          ...
          <div className="results-container">
            ...
            {isSolo && (
              <div className="winner-section">
                ...
                <h3 className="winner-username">{currentUser.username}</h3>
              </div>
            )}
              ...
              <div className="draw-section">
                <div className="draw-avatars">
                  <div className="draw-player">
                    {currentUser.profilePicture ? (
                      <img src={currentUser.profilePicture} alt="You" className="draw-avatar" />
                    ) : (
                  ...
                  <div className="draw-player">
                    {opponent.profilePicture ? (
                      <img src={opponent.profilePicture} alt="Opponent" className="draw-avatar" />
                    ) : (
                      <div className="draw-avatar-placeholder">
            ...
            <div className="scores-display">
              <h2 className="score-title">Final Score: {userScore}/{totalQuestions}</h2>
            </div>
